...
describe User do
 ...
 describe "when email address is already taken" do
  ...
  #Should fail if PAssword in not present
  ...
  describe "return value of authenticate method" do
    before { @user.save } #saves the user to the database so that is can be retrieved by find_by 
    let(:found_user) { User.find_by(email: @user.email) } #sets the local variable found_user = to the output of the do...end statement 
    ...
    describe "with invalid password" do
      ...
      specify { expect(user_for_invalid_password).to be_false } #specify is similar to it; see page 304 
    end
